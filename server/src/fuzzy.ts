/** One-edit spelling correction, run only when an exact search found nothing. */
import type { Artifact } from './artifact.js';

/** Below this, correction is guesswork rather than inference: "brno" is one
 * edit from "brna", "brod" and "bruno" alike. */
const MIN_LENGTH = 5;

/** The prefix half of a five-character token is three characters and can span a
 * large slice of the dictionary. The length filter rejects most of those for the
 * price of a subtraction; this bounds the rest. */
...
export function withinOneEdit(q: string, t: string): boolean {
  ...
  while (i < lq && j < lt) {
    ...
    // An insert or delete advances only the longer side, which realigns the rest.
 ...
 * distance 1 either starts with its first half or ends with its second. Both are
 * prefix searches the dictionary already does by binary search, the second on
 * the reversed copy. SymSpell would want a 40MB delete table for the same job.
 */
 ...
 * the same. Null leaves the token as typed.
 */
export function correctToken(a: Artifact, token: string): string | null {
  ...
  for (const id of neighbours(a, token)) {
    ...
    // Ties break on the term itself, not on dictionary order.
